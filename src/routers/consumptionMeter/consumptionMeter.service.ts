...
export class ConsumptionMeterService {
  ...
  public async upload(dto: UploadConsumptionMeterDto) {
    ...
    if (!base64Regex.test(dto.image)) throw new BadRequestException({ error_code: "INVALID_DATA", error_description: "Imagem deve esta em base64" });

    const format = "YYYY-MM-DD HH:mm:ss";
    const date = dayjs(dto.measure_datetime, format, true);
    if (!date.isValid()) throw new BadRequestException({ error_code: "INVALID_DATA", error_description: "A data deve esta no formato YYYY-MM-DD HH:mm:ss" });

    if (dto.measure_type !== "WATER" && dto.measure_type !== "GAS") {
      throw new BadRequestException({ error_code: "INVALID_DATA", error_description: "Tipo de leitura: WATER ou GAS" });
    }
    ...
    if (existingMeasurement.length > 0) throw new ConflictException({ error_code: "DOUBLE_REPORT", error_description: "Leitura do mês já realizada" });
  ...
  private async sentImageToGemini(filePath: string, format: string, filename: string) {
    ...
    const objectResponse = {
      ...
      measure_value: Number(result.response.text().replace(/\n/g, "").trim()),
  ...
  private async saveBase64Image(base64Image: string): Promise<{ filename: string; format: string; filePath: string }> {
    try {
  ...
  public async confirm(dto: ConfirmConsumptionMeterDto) {
    if (typeof dto.confirmed_value !== "number") throw new BadRequestException({ error_code: "INVALID_DATA", error_description: "O valor tem que ser um número" });
    if (typeof dto.measure_uuid !== "string") throw new BadRequestException({ error_code: "INVALID_DATA", error_description: "O valor tem que ser um string" });
    ...
    if (!measurement) throw new NotFoundException({ error_code: "MEASURE_NOT_FOUND", error_description: "Leitura do mês já realizada" });
    if (measurement.has_confirmed == true) throw new ConflictException({ error_code: "CONFIRMATION_DUPLICATE", error_description: "Leitura do mês já realizada" });
  ...
  public async list(customer_code: string, query: any) {
    let measures = null;

    if (query.measure_type) {
      if (query.measure_type !== "WATER" && query.measure_type !== "GAS") {
        throw new BadRequestException({ error_code: "INVALID_TYPE", error_description: "Tipo de medição não permitida" });
      }
    ...
    if (!measures.length) throw new NotFoundException({ error_code: "MEASURES_NOT_FOUND", error_description: "Nenhuma leitura encontrada" });
